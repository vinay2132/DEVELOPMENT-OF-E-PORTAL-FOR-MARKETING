...
if($num==1){
    echo '<script>alert("Someone entrepreneurs has already register using this email")</script>';
...
$nums=mysqli_num_rows($results);
 if($nums == 1){
  echo '<script>alert("Someone mentor has already register using this email")</script>';
}
...
move_uploaded_file($tname1, $uploads_dir.'/'.$pname1);
     ...
     $reg="INSERT Into mentorship (full_name,email, phone,  password,linkedinlink,skills,expertise,experience,message,file_memo,vkey,varefied) values('$full_name','$email','$phone','$password','$linkedinlink','$skills','$expertise','$experience','$message','$pname1','$vkey','$varefied')";
     mysqli_query($db,$reg);
...
$alert = '';
  ...
  try{
    ...
    $mail->Body = "<h3>Name : $full_name <br>Email: $email <br>Message : $vkey</h3><a href='http://localhost/pp/college_majorproject/general_homepage/innerhtml/enterpreners/enterpreners-inside/enterprenersignin/mentorshipvarified.php?vkey=$vkey'>register account</a>";
