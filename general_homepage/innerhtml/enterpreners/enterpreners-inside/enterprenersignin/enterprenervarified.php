...
if(isset($_GET['vkey'])){
    ...
    $s ="select * from entrepreneurs where varefied=0 && vkey='$vkey'";
    $result = mysqli_query($db,$s);
    $num=mysqli_num_rows($result);
    if($num==1){
       // $update $mysqli->query("update companyregs set varefied=1 where vkey = '$vkey' limit 1");
       $ss="update entrepreneurs set varefied=1 where vkey = '$vkey'";
       mysqli_query($db,$ss);
       if($ss){
        $sql = "select * from entrepreneurs where vkey = '$vkey'";
